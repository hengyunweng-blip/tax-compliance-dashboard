...
export function PersonalSummary({ summary }: { summary: PersonalTaxSummary }) {
  return (
    <section className="annual-worksheet-card" data-testid="personal-worksheet">
      ...
      {summary.trustDistributions.length ? <section className="annual-distributions"><h3>信托分配来源</h3><ul>{summary.trustDistributions.map((distribution) => <li key={distribution.id}>{distribution.trustEntityName} · {formatCents(distribution.amountCents)} · {distribution.statusLabel} · 决议日 {formatDueDate(distribution.resolutionDate)}</li>)}</ul></section> : null}
