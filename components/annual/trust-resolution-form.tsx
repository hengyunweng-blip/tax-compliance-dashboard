...
import { TrustDistributionEditor } from "@/components/annual/trust-distribution-editor";
...
export function TrustResolutionForm({ draft }: { draft: TrustDistributionDraft }) {
  return (
    <section className="annual-worksheet-card" data-testid="trust-worksheet">
      ...
      <div className="annual-metric-grid"><div><span>可分配收入（不含 GST，计算草稿）</span><strong>{annualMoney(draft.distributableIncomeCents)}</strong></div><div><span>总折旧额（不含 GST）</span><strong>{annualMoney(draft.depreciationCents)}</strong></div><div><span>可抵扣折旧额（按私人使用调整，不含 GST）</span><strong>{annualMoney(draft.deductibleDepreciationCents)}</strong></div><div><span>受益人分配合计</span><strong>{formatCents(draft.distributionTotalCents)}</strong></div></div>
      {draft.distributionDifferenceCents !== null && draft.distributionDifferenceCents !== 0 ? <p className="annual-independent-note danger-text">分配合计与可分配收入差额：{formatCents(draft.distributionDifferenceCents)}；必须说明，不自动配平。</p> : null}
      ...
      <TrustDistributionEditor draft={draft} />
