...
export function NewsPageClient({ initialItems, initialSources, aiEnabled, aiStatus }: Props) {
  ...
  async function dismiss(id: number) {
    ...
    const payload = await response.json() as { items: NewsFeedItem[]; sources: NewsSource[] };
  ...
  async function createTodo(analysisId: number) {
    ...
    const payload = await response.json() as { items?: NewsFeedItem[]; sources?: NewsSource[]; error?: string };
  ...
  return (
    <main className="ledger-shell" data-testid="news-page">
      ...
      <section className="ledger-content"><header className="ledger-header"><div><p className="page-kicker">Gate 4 · 资讯与人工确认</p><h1>监管资讯</h1><p>首屏读取本地缓存；外部来源后台刷新，单源失败不会阻塞看板或账本。</p></div><div className="news-header-actions"><span className="ai-status" data-testid="ai-status">{aiStatus}</span><button type="button" className="secondary-button" onClick={() => void refresh()} disabled={busy}>{busy ? "启动中…" : "后台刷新"}</button></div></header>
        ...
        <section className="news-feed" aria-label="资讯列表"><div className="inbox-section-heading"><div><p className="page-kicker">关键词预筛</p><h2>相关资讯</h2></div><span>{items.length} 条</span></div>{items.length ? items.map((item) => <NewsCard key={item.id} item={item} onDismiss={(id) => void dismiss(id)} onCreateTodo={(id) => void createTodo(id)} confirming={confirming === item.analysisId} />) : <p className="empty-state">当前没有缓存资讯；可启动后台刷新。AI 状态：{aiEnabled ? "已启用" : "已关闭"}。</p>}</section>
