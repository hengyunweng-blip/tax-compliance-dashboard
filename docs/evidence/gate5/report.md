...
日期：2026-08-27（Australia/Melbourne）

Gate 4 已验收并保留 `gate-4` 标签；Gate 5 本次只完成实现和验证，尚未打 `gate-5` 标签，等待用户验收。
...
- 每个年度底稿都列出五项待人工补充：折旧、结转亏损、franking account 余额、Div 7A 借款余额、信托 FTE 状态。
- 养老金当前上限为 `3,000,000` 分（$30,000.00），来源为 [ATO caps, limits and tax on super contributions](https://www.ato.gov.au/individuals-and-families/super-for-individuals-and-families/super/growing-and-keeping-track-of-your-super/caps-limits-and-tax-on-super-contributions)，取数日期 `2026-08-27`。
...
```text
npm test -- --run       29 个测试文件，140 个测试全部通过
npm run lint            通过
npm run build           通过，Next.js production build/typecheck 通过
Playwright final-regression.spec.ts  1 passed
```

最终浏览器回归使用独立数据库 `data/gate5-evidence-final3.db`，截图只写入 Gate 5 目录：

- `annual-worksheets.png`
- `div7a-official-baseline.png`
- `super-backup.png`

浏览器验证包含年度切换、官方 Div 7A 样本、养老金双任务、备份 ZIP 响应和 390px 窄屏无横向溢出。未声称验证真实手机摄像头或手机拍照上传。
