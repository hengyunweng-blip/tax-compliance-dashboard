...
新扩展沿用严格逐 Gate 停止。现有 Gate 0–5 是历史基线；本轮只提交设计和计划，不开始 Gate 5 修复或 Gate 6。每个未来 Gate 交付自己的测试、API/SQL/UI 证据和报告后立即停止，只有用户明确验收才进入下一 Gate。

### Gate 5 基线收口（既有授权，不是新增范围）

- 年度收入、运营费用、资本采购全部按不含 GST；四季 BAS 的 `G1 - 1A` 与年度收入一致。
- Div 7A 余额逐年推进，期初余额不再每年恒为本金；官方基准仍来自 ATO fixture。
- `lodged_at`/`paid_at` 使用用户录入日期并实际写入。
- Gate 5 不创建标签，直到用户验收。

### Gate 6 — Div 7A 完整化（最高优先级）

- 年度基准利率表、来源/取数日期和缺失安全行为。
...
- 一张资产表、整数分、手动有效年限、两种方法、私人使用比例、累计折旧/账面余额。
- 年度底稿按 income_year 接入折旧，并标注轻量计算/人工核对。
- 车辆作为资产登记数据来源，但不在本 Gate 自动选择 FBT 或 Div 7A。

### Gate 8 — 公司车辆 FBT/Div 7A 事实分析（只写分析，不写自动判定代码）

- 真实 ATO 来源、FBT 年度/截止日/12 周 logbook、Div 7A asset-use 条件、证据清单和并列结果模板。
- 交付一份用户可填写的事实清单，明确哪些资料不足以判断。
- 不插入 FBT 义务、不宣称哪条路径适用；若用户之后确认 FBT 适用，再按独立变更实现 FBT 规则。

### Gate 9 — PSI 定性向导

- 每家公司/测试个人的来源录入、results test、80% rule、unrelated clients test 和三态结果。
- PSI rules 适用或关键输入 unknown 时，规划门控行为。
- 不做未列入本需求的工资、STP 或完整 PSI 政策数据库。

### Gate 10 — 三情景保守规划和 AI 解释层

- 只实现保留公司利润、增加养老金、偿还 Div 7A。
- 输入完整性、来源/假设/适用年度/排除项、not-applicable 和 manual-review 状态。
- AI 只整理已确定结果；用户确认写独立快照/待办/审计，不改底稿。

### Gate 11 — 跨模块回归和手动申报工作台

- 回归提醒、ICS、CAV/ASIC、操作指引、ATO 资讯、年度底稿、备份还原和窄屏布局。
- 如果用户在 Gate 8 后明确某公司 FBT 适用，才在本 Gate 以独立 FBT 年度和截止日实现 `fbt_annual_return`；未确认时保持 unknown，不生成卡片。
