...
**状态：已按用户最新确认修订设计；车辆事实清单已交付；Gate 5 三项基线修复已完成；Gate 6 已验收；Gate 7 A/B 实现与证据进行中**
...
- A：从 ATO 官方资料核对 s109R 的重借风险；只提供可配置窗口的风险提示，不自动认定、扣除或创建分红记录；用户复核选择写入审计。
- A：同一贷款方、借款人、所得年度和相同最高期限的贷款在页面合并展示并合计最低还款；协议义务仍按每笔贷款独立追踪。
- B：实现一张轻量资产表，成本为不含 GST 分，支持手动有效年限、prime cost/diminishing value、私人使用比例、30 Jun 2026 期初余额、首年/处置年按天数比例和年度底稿接入。
- B：期初资料缺失时显示“无法判断 / 期初余额未配置”，不假设零值或成本；车辆只链接事实清单并提示可能存在 FBT/Div 7A 后果，不实现 FBT 计算。
- 车辆事实清单已提前交付；车辆 FBT/Div 7A 事实分析仍是后续资料工作，不在本 Gate 自动选择路径。
...
### Gate 9 — 轻量资产登记与折旧（已前移并纳入 Gate 7；本节保留为设计索引）

- 该范围已在 Gate 7 实现：一张资产表、整数分、手动有效年限、两种方法、私人使用比例、累计折旧/账面余额；支持从 30 Jun 2026 期初累计折旧和账面余额接续。
