...
export function exportBasPdf(worksheetId: number): Response {
  ...
  const lines = [
    ...
    `PAYG instalment: ${worksheet.paygInstalmentCents === null ? "Not entered" : formatCents(worksheet.paygInstalmentCents)}`,
    `Statement total: ${worksheet.statementTotalCents === null ? "Not resolved" : formatCents(worksheet.statementTotalCents)}`,
    "",
